document.addEventListener("DOMContentLoaded", function() {
    ...
    const productsPerPage = 24; // Số lượng sản phẩm trên mỗi trang

    loadCategory();

    if (categoryId) {
        updateBreadcrumb(categoryId);
        loadProducts(categoryId, currentPage, productsPerPage); // Load sản phẩm theo danh mục
    } else {
        updateBreadcrumb(null);
        loadProducts(null, currentPage, productsPerPage); // Load tất cả sản phẩm
    }
...
function loadProducts(categoryId = null, page = 1, limit = 12) {
    let url = `php/getProducts.php?page=${page}&limit=${limit}`;
    if (categoryId) {
    ...
    Promise.all([fetch(url), fetchFavorites(), fetchOrders()])
        ...
        .then(([products, favoritesCounts, ordersCounts]) => {
            ...
            const totalProducts = products.length > 0 ? products[0].total_rows : 0;
...
function updatePagination(totalPages) {
    ...
    pagination.innerHTML += `<li><a href="#" id="first-page">«</a></li>`;

    for (let i = 1; i <= totalPages; i++) {
        pagination.innerHTML += `<li><a href="#" class="page-link">${i}</a></li>`;
    }

    pagination.innerHTML += `<li><a href="#" id="last-page">»</a></li>`;
    ...
    document.getElementById('first-page').addEventListener('click', (event) => {
        event.preventDefault();
        currentPage = 1;
        loadProducts(categoryId, currentPage, productsPerPage);
        updateActivePage(currentPage);
    });

    document.getElementById('last-page').addEventListener('click', (event) => {
        event.preventDefault();
...
// Xử lý sự kiện khi click vào button kích cỡ
